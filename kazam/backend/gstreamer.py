...
class Screencast(GObject.GObject):
    ...
    def setup_sources(self,
        ...
        logger.debug("Recording - Capture Cursor: {0}".format(capture_cursor))
        logger.debug("Recording - Framerate : {0}".format(capture_cursor))
    ...
    def setup_video_source(self):
        ...
        logger.debug("Recording - Coordinates: {0} {1} {2} {3}".format(startx, starty, endx, endy))
        ...
        if self.codec == CODEC_VP8:
            logger.debug("Recording - Codec: VP8/WEBM")
            ...
            logger.debug("Recording - Codec: H264/MATROSKA")
    ...
    def setup_audio_source(self):
        logger.debug("Recording - Audio1 Source:\n  {0}".format(self.audio_source))
    ...
    def setup_audio2_source(self):
        logger.debug("Recording - Audio2 Source:\n  {0}".format(self.audio2_source))
    ...
    def setup_filesink(self):
        logger.debug("Recording - Filesink: {0}".format(self.tempfile))
    ...
    def setup_pipeline(self):
        if self.video_source and not self.audio_source and not self.audio2_source:
            logger.debug("Recording - Pipline - Video only".format(self.audio_source))
            ...
            logger.debug("Recording - Pipline - Video + Audio".format(self.audio_source))
            self.pipeline.add(self.videosrc, self.vid_in_queue, self.videorate,
                              self.vid_caps_filter, self.ffmpegcolor,
            ...
            logger.debug("Recording - Pipline - Video + Dual Audio".format(self.audio_source))
            self.pipeline.add(self.videosrc, self.vid_in_queue, self.videorate,
    ...
    def start_recording(self):
        if self.debug:
            logger.debug("Recording - Generating dot file.")
        ...
        logger.debug("Recording - Setting STATE_PLAYING")
        self.pipeline.set_state(gst.STATE_PLAYING)

    def pause_recording(self):
        logger.debug("Recording - Setting STATE_PAUSED")
        self.pipeline.set_state(gst.STATE_PAUSED)

    def unpause_recording(self):
        logger.debug("Recording - Setting STATE_PLAYING - UNPAUSE")
        self.pipeline.set_state(gst.STATE_PLAYING)

    def stop_recording(self):
        logger.debug("Recording - Sending new EOS event")
        self.pipeline.send_event(gst.event_new_eos())
        #
        # TODO: Improve this ;)
        #
        if self.debug:
            logger.debug("Recording - Generating PNG file from DOT file.")
            if os.path.isfile("/usr/bin/dot"):
                ...
                logger.debug("Recording - 'dot' not found. Unable to generate PNG.")
                pass
    ...
    def on_message(self, bus, message):
        ...
        if t == gst.MESSAGE_EOS:
            ...
            logger.debug("Recorder - Received EOS, setting pipeline to NULL.")
        elif t == gst.MESSAGE_ERROR:
            logger.debug("Recorder - Received an error message.")
